...
def create_monthly_cash_flow(df_entradas, df_saidas, saldo_inicial, projection_months=3):
    ...
    try:
        ...
        # Adicionar coluna com valores absolutos das saídas para o gráfico
        ...
        if len(fluxo) > 0:
            ...
            for i in range(1, projection_months + 1):
                ...
                projection.append({
                    ...
                    'Saidas_abs': abs(projected_saidas),
...
if df_entradas is not None and df_saidas is not None and len(df_entradas) > 0 and len(df_saidas) > 0:
    ...
    if fluxo is not None and len(fluxo) > 0:
        ...
        with tab1:
            ...
            # Barras de saídas - USANDO VALORES ABSOLUTOS PARA FICAREM PARA CIMA
            fig.add_trace(go.Bar(
                x=fluxo['Mês/Ano'],
                y=fluxo['Saidas_abs'],  # Usando a coluna com valores absolutos
                name='Saídas',
                marker_color='red',
                opacity=0.7,
                width=0.3
            ...
            # Formatar valores
            fluxo_display['Entradas'] = fluxo_display['Entradas'].apply(lambda x: f"R$ {x:,.2f}")
            fluxo_display['Saídas'] = fluxo_display['Saídas'].apply(lambda x: f"R$ {x:,.2f}")
        ...
        with tab2:
            if projecao_diaria is not None and len(projecao_diaria) > 0:
                ...
                st.info("Conteúdo da Projeção Diária")
            else:
        ...
        with tab3:
            ...
            # ... (código da tab3)
            st.info("Conteúdo da Análise por Empresa")
        
        with tab4:
            st.subheader("Últimas Transações")
            # ... (código da tab4)
            st.info("Conteúdo das Últimas Transações")
