...
@st.cache_data
def load_data():
    # Dados do faturamento histórico
    historico_data = {
        'EMPRESA': ['PRO CLEAN'] * 58,
        'COD': [1389, 1389, 1389, 1389, 1389, 1389, 1389, 1389, 1389, 1389, 1389, 1389, 1389,
                5179, 5179, 5179, 5179, 5179, 5179, 5179, 5179, 5179, 5179, 5179, 5179, 5179,
                999259, 999259, 999259, 999259, 999259, 999259, 999259, 999259, 999259, 999259, 999259, 999259, 999259,
                1914, 1914, 1914, 1914, 1914, 1914, 1914,
                4113, 4113, 4113,
                1501, 1501, 1501, 1501, 1501, 1501, 1501, 1501],
        'CLIENTE': [
            '1300 JURUPIS', '1300 JURUPIS', '1300 JURUPIS', '1300 JURUPIS', '1300 JURUPIS', 
            '1300 JURUPIS', '1300 JURUPIS', '1300 JURUPIS', '1300 JURUPIS', '1300 JURUPIS',
            '1300 JURUPIS', '1300 JURUPIS', '1300 JURUPIS',
            '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA',
            '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA',
            '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA',
            '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA', '14º CARTORIO DA LAPA',
            '14º CARTORIO DA LAPA',
            '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO',
            '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO',
            '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO',
            '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO', '32º CARTORIO DO SOCORRO',
            '32º CARTORIO DO SOCORRO',
            'ABOUT VILA MARIANA', 'ABOUT VILA MARIANA', 'ABOUT VILA MARIANA',
            'ABOUT VILA MARIANA', 'ABOUT VILA MARIANA', 'ABOUT VILA MARIANA',
            'ABOUT VILA MARIANA',
            'ACQUA PARK BETHAVILLE', 'ACQUA PARK BETHAVILLE', 'ACQUA PARK BETHAVILLE',
            'AD 330 ALTO DA BOA VISTA', 'AD 330 ALTO DA BOA VISTA', 'AD 330 ALTO DA BOA VISTA',
            'AD 330 ALTO DA BOA VISTA', 'AD 330 ALTO DA BOA VISTA', 'AD 330 ALTO DA BOA VISTA',
            'AD 330 ALTO DA BOA VISTA', 'AD 330 ALTO DA BOA VISTA'
        ],
        'DTVENCIMENTO': [
        ...
        'VALORLIQUIDO': [
            9676.29, 9676.29, 9676.29, 9676.29, 9676.29, 9676.29, 9676.29, 9676.29,
            9676.29, 9676.29, 9676.29, 9676.29, 10437.82,
            6909.33, 6909.33, 6909.33, 6909.33, 6909.33, 6909.33, 6909.33, 6909.33,
            6909.33, 6909.33, 6909.33, 6909.33, 7453.09,
            ...
            26939.92, 29686.02, 29284.22,
            24116.09, 28199.87, 28199.87, 28199.87, 28199.87, 28199.87, 28199.87, 28199.87
        ]
    }
    ...
    df_historico['DATA_DEPOSITO'] = df_historico['DTVENCIMENTO'].apply(
        lambda x: get_deposit_date(datetime.combine(x, datetime.min.time())).date()
    )
...
with tab2:
    ...
    # Agrupar por mês
    df_filtrado['Mês'] = pd.to_datetime(df_filtrado['DATA_DEPOSITO']).dt.strftime('%B')
    df_filtrado['Ano'] = pd.to_datetime(df_filtrado['DATA_DEPOSITO']).dt.year
    ...
    df_filtrado['Mês_PT'] = df_filtrado['Mês'].map(meses_pt)
    
    # Criar tabela pivô
    pivot_mensal = df_filtrado.pivot_table(
        values='VALORLIQUIDO',
        index='CLIENTE',
        columns='Mês_PT',
        aggfunc='sum',
...
with tab3:
    ...
    # Criar range de datas para projeção
    data_inicio_proj = datetime(2026, 1, 1).date()
    data_fim_proj = datetime(2026, 3, 31).date()
    ...
    # Preencher entradas baseado no histórico projetado
    ...
    fig.update_layout(
        title='Saldo e Entradas Diárias',
        xaxis_title='Data',
        yaxis_title='Valor (R$)',
        hovermode='x unified'
    )
...
with tab4:
    ...
    with st.form("form_pagamento"):
        ...
        with col1:
            ...
            valor_pag = st.number_input("Valor (R$)", min_value=0.0, step=100.0)
...
<div style='text-align: center'>
    ...
    <p style='font-size: 12px; color: gray;'>Considere D+2 dias úteis para compensação</p>
</div>
